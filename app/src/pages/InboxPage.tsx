...
import { LayoutGrid, List, Loader2, Inbox as InboxIcon } from 'lucide-react'
...
// Reusable Kanban column for filtered views
...
export function InboxPage() {
  ...
  // Filter: unassigned tickets only (excluding done/cancelled)
  ...
  // Group by stage for mini kanban
  const ticketsByStage = useMemo(() => {
    const stages: TicketStage[] = ['new', 'assigned', 'in_progress', 'pending_dev', 'testing', 'pending_validation']
        ...
        <main className="flex-1 overflow-hidden flex flex-col">
          <header className="h-16 border-b border-[#E0E0E1] flex items-center justify-between px-6 shrink-0 bg-white">
            <div className="flex items-center gap-3">
              <InboxIcon className="w-5 h-5 text-[#6353FF]" />
              <h1 className="text-lg font-semibold text-[#3F4444]">Inbox</h1>
              <span className="text-sm text-[#8A8F8F]">({tickets.length} sin asignar)</span>
            </div>
